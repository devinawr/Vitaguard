...
@section('content')
@php
    $isThumbnailLink = $article->thumbnail && \Illuminate\Support\Str::startsWith($article->thumbnail, ['http://', 'https://']);

    $currentThumbnailType = old('thumbnail_type', $isThumbnailLink ? 'link' : 'upload');

    $currentThumbnailLink = old('thumbnail_link', $isThumbnailLink ? $article->thumbnail : '');

    $currentThumbnailPreview = old('thumbnail_link')
        ? old('thumbnail_link')
        : ($article->thumbnail
            ? ($isThumbnailLink ? $article->thumbnail : asset('storage/' . $article->thumbnail))
            : 'https://placehold.co/800x500/fdeef1/ef8aa0?text=Preview+Thumbnail');
@endphp
        ...
        <div class="card">
            ...
            <form action="{{ route('admin.articles.update', $article) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-check form-check-inline">
                            <input
                                ...
                                value="upload"
                                {{ $currentThumbnailType === 'upload' ? 'checked' : '' }}
                            >
                            <label class="form-check-label" for="thumbnail_type_upload">
                                Upload File
                            </label>
                        </div>
                        ...
                        <div class="form-check form-check-inline">
                            <input
                                ...
                                value="link"
                                {{ $currentThumbnailType === 'link' ? 'checked' : '' }}
                            >
                            <label class="form-check-label" for="thumbnail_type_link">
                                Pakai Link
                            </label>
                        </div>
                    ...
                    <div class="mb-3 {{ $currentThumbnailType === 'link' ? 'd-none' : '' }}" id="thumbnail-upload-group">
                        <label for="thumbnail_file" class="form-label">Upload Thumbnail</label>

                        @if($article->thumbnail && !$isThumbnailLink)
                            <div class="mb-2">
                                <small class="text-muted d-block mb-2">Thumbnail saat ini:</small>
                                <img
                    ...
                    <div class="mb-3 {{ $currentThumbnailType === 'link' ? '' : 'd-none' }}" id="thumbnail-link-group">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            >
                                ...
                                <option value="archive" {{ old('status', $article->status) == 'archive' ? 'selected' : '' }}>
                                    Archive
                                </option>
                            </select>
                        ...
                        <div
                            class="col-md-6 mb-3 {{ old('status', $article->status) == 'draft' ? 'd-none' : '' }}"
                            id="published-at-group"
                        >
                            ...
                            <small class="text-muted">
                                Kosongkan jika ingin artikel langsung tayang saat disimpan. Isi tanggal & jam ke depan untuk menjadwalkan publish nanti.
                            </small>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const existingPreview = @json($currentThumbnailPreview);
    ...
    function toggleThumbnailInput() {
        if (uploadRadio.checked) {
            ...
            if (!fileInput.files.length) {
                preview.src = existingPreview || placeholder;
            }
        } else {
            uploadGroup.classList.add('d-none');
            linkGroup.classList.remove('d-none');
            preview.src = linkInput.value.trim() || existingPreview || placeholder;
        }
    }
    ...
    linkInput.addEventListener('input', function () {
        preview.src = this.value.trim() || existingPreview || placeholder;
    });
